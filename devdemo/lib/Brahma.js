...
/**
@method percentify
conver pixels to percents
*/
Brahma.percentify = function(px, mpx) {
	if ("string" === typeof px) {
		if (px.substr(-2)==='px') {
			return ((parseFloat(px)/parseFloat(mpx))*100).toFixed(2);
		} else if (px.substr(-3)==='rat') {
			return (parseFloat(mpx)*parseFloat(px)).toFixed(2)
		} else {
			return parseFloat(px).toFixed(2);
		}
	} else {
		return ((px/parseFloat(mpx))*100).toFixed(2);
	}
}
...
Brahma.vector.html = function(html) {
	if ("undefined"===typeof html) {
		...
		return Brahma.bench(this, Array.from(arguments), function(elem) {
